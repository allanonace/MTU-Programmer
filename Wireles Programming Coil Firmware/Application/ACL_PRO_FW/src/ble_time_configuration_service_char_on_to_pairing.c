...
*
* Module Description:
* Module for managing the characteristic on_to_pairing
*
***************************************************/
...
uint32_t  current_on_to_pairing;
...
uint32_t ble_time_configuration_service_on_to_pairing_add(ble_time_configuration_service_t *p_time_configuration_service, const ble_time_configuration_service_init_t *p_time_configuration_service_init)
{
    ...
    ble_gatts_attr_md_t attr_md;
	  uint8_t             initial_on_to_pairing[LEN_ON_TO_PAIRING_CHAR];
  
    current_on_to_pairing = button_management_get_on_to_pairing_value();
  
    uint8_t   num_shift = 0;
    for(uint8_t i=0;i<LEN_ON_TO_PAIRING_CHAR;i++)
    {
        num_shift = 8*(LEN_ON_TO_PAIRING_CHAR-i-1);
        initial_on_to_pairing[i]  =  current_on_to_pairing>>num_shift;
    }
    ...
    ble_uuid.uuid = BLE_UUID_ON_TO_PAIRING_CHAR;
    ...
    attr_char_value.init_len     = LEN_ON_TO_PAIRING_CHAR;
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = LEN_ON_TO_PAIRING_CHAR;
    attr_char_value.p_value      = initial_on_to_pairing;

		return sd_ble_gatts_characteristic_add(p_time_configuration_service->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_time_configuration_service->time_configuration_on_to_pairing);																		 
																							 
}

/**************************************************
* Function name	: uint32_t on_to_pairing_update(ble_time_configuration_service_t * p_time_configuration_service, 
*                 uint8_t *data)                 
*    returns		: NRF_SUCCESS on success, otherwise an error code.
*    arg1			: pointer to TIME Configuration Service structure
*    arg2			: uint8_t new on_to_pairing
* Created by		: Nordic (modified by María Viqueira)
* Date created		: 2018/07/03
* Description		: updates the characteristic on_to_pairing with the new value
* Notes			: restrictions, odd modes
**************************************************/  
uint32_t on_to_pairing_update(ble_time_configuration_service_t * p_time_configuration_service, uint8_t *data)
{
			uint32_t err_code = NRF_SUCCESS;
			uint16_t len = LEN_ON_TO_PAIRING_CHAR;
			...
			err_code = sd_ble_gatts_value_set(p_time_configuration_service->conn_handle,
                                          p_time_configuration_service->time_configuration_on_to_pairing.value_handle,
                                          &gatts_value);
			 ...
			 if (err_code != NRF_SUCCESS)     
				...
				hvx_params.handle = p_time_configuration_service->time_configuration_on_to_pairing.value_handle;
...
void ble_time_configuration_char_on_to_pairing_handler(ble_time_configuration_service_t * p_time_configuration, uint8_t * new_state)
{
    ...
    if(connection_control_management_get_connection_valid())
    {
      ...
      //Check thresholds
      if( (timeout  <=  BUTTON_MANAGEMENT_MAX_THRES_ON_TO_PAIRING)  &&  (timeout  >=  BUTTON_MANAGEMENT_MIN_THRES_ON_TO_PAIRING))   
      {
          //At least one second higher than power_off
          if(timeout  >=  (BUTTON_MANAGEMENT_MIN_DISTANCE_BETWEEN_PULSE  + button_management_get_power_off_value()))
          {
            current_on_to_pairing = timeout;
    ...
    {
        num_shift = 8*(LEN_ON_TO_PAIRING_CHAR-i-1);
        new_state[i]  =  current_on_to_pairing>>num_shift;
    }
    
    button_management_new_on_to_pairing_value(current_on_to_pairing);
    on_to_pairing_update(p_time_configuration,new_state);
