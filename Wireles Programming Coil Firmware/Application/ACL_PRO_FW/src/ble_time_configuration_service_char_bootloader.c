...
*
* Module Description:
* Module for managing the characteristic bootloader
*
***************************************************/
...
uint32_t current_bootloader_timeout;
...
uint32_t ble_time_configuration_service_bootloader_add(ble_time_configuration_service_t *p_time_configuration_service, const ble_time_configuration_service_init_t *p_time_configuration_service_init)
{
    ...
    ble_gatts_attr_md_t attr_md;
	  uint8_t             initial_bootloader[LEN_BOOTLOADER_CHAR];

    current_bootloader_timeout  = flash_management_get_bootloader_timeout();
    uint8_t   num_shift = 0;
    for(uint8_t i=0;i<LEN_BOOTLOADER_CHAR;i++)
    {
        num_shift = 8*(LEN_BOOTLOADER_CHAR-i-1);
        initial_bootloader[i]  =  current_bootloader_timeout>>num_shift;
    }
    ...
    ble_uuid.uuid = BLE_UUID_BOOTLOADER_CHAR;
    ...
    attr_char_value.init_len     = LEN_BOOTLOADER_CHAR;
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = LEN_BOOTLOADER_CHAR;
    attr_char_value.p_value      = initial_bootloader;

		return sd_ble_gatts_characteristic_add(p_time_configuration_service->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_time_configuration_service->time_configuration_bootloader);																		 
																							 
}

/**************************************************
* Function name	: uint32_t bootloader_timeout_update(ble_time_configuration_service_t * p_time_configuration_service, 
*                 uint8_t *data)                 
*    returns		: NRF_SUCCESS on success, otherwise an error code.
*    arg1			: pointer to TIME Configuration Service structure
*    arg2			: uint8_t new bootloader_timeout
* Created by		: Nordic (modified by María Viqueira)
* Date created		: 2018/07/03
* Description		: updates the characteristic bootloader_timeout with the new value
* Notes			: restrictions, odd modes
**************************************************/  
uint32_t bootloader_timeout_update(ble_time_configuration_service_t * p_time_configuration_service, uint8_t *data)
{
			uint32_t err_code = NRF_SUCCESS;
			uint16_t len = LEN_BOOTLOADER_CHAR;
			...
			err_code = sd_ble_gatts_value_set(p_time_configuration_service->conn_handle,
                                          p_time_configuration_service->time_configuration_bootloader.value_handle,
                                          &gatts_value);
			 ...
			 if (err_code != NRF_SUCCESS)     
				...
				hvx_params.handle = p_time_configuration_service->time_configuration_bootloader.value_handle;
...
void ble_time_configuration_char_bootloader_handler(ble_time_configuration_service_t * p_time_configuration, uint8_t * new_state)
{
    ...
    if(connection_control_management_get_connection_valid())
    {
      for(uint8_t i=0;i<LEN_BOOTLOADER_CHAR;i++)
      {
       ...
       //Check thresholds
      if( (timeout  <=  FLASH_MANAGEMENT_MAX_THRES_ENTER_BOOTLOADER)  &&  (timeout  >=  FLASH_MANAGEMENT_MIN_THRES_ENTER_BOOTLOADER)) 
      {
          //Mandatory: Bootloader >= cancel + wake up
          if( timeout  > (flash_management_get_cancel_bootloader_timeout() + flash_management_get_wake_up_from_sleep_timeout())  )
          {
            current_bootloader_timeout = timeout;
    ...
    {
        num_shift = 8*(LEN_BOOTLOADER_CHAR-i-1);
        new_state[i]  =  current_bootloader_timeout>>num_shift;
    }
    
    bootloader_timeout_update(p_time_configuration,new_state);
    flash_management_new_bootloader_timeout(current_bootloader_timeout);
